...
export default function Contact() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <div className="grid lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-2 space-y-8">
              <AnimatedSection direction="left">
                <span className="text-[#D32F2F] font-mono text-sm tracking-widest uppercase block mb-6">Location</span>
                ...
                <div className="map-placeholder rounded-2xl h-48 flex items-center justify-center border border-white/10 relative overflow-hidden">
                  <div className="relative z-10 text-center">
                    ...
                    <a
                      ...
                      className="text-[#D32F2F] text-xs mt-2 inline-block hover:underline"
                    >
                      Open in Google Maps →
                    </a>
                  </div>
              ...
              <AnimatedSection direction="left" delay={0.2}>
                <div className="space-y-4">
                  {contacts.map(c => (
                    <div key={c.label} className="flex items-start gap-3 bg-[#003366] border border-white/5 rounded-xl p-4">
                      <div className="w-8 h-8 rounded-lg bg-[#D32F2F]/10 flex items-center justify-center shrink-0">
                        <c.icon size={14} className="text-[#D32F2F]" />
                      </div>
                      <div>
                ...
                <div className="flex gap-3">
                  {[Facebook, Twitter, Linkedin, Youtube, Instagram].map((Icon, i) => (
                    <a key={i} href="#" className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-gray-500
                                                    hover:border-[#D32F2F] hover:text-[#D32F2F] transition-all duration-300">
                      <Icon size={15} />
                    </a>
                  ))}
                </div>
            ...
            <div className="lg:col-span-3">
              <AnimatedSection direction="right">
                {submitted ? (
                  <motion.div
                    className="bg-[#003366] border border-green-500/30 rounded-2xl p-12 text-center"
                  ...
                  <form onSubmit={handleSubmit} className="bg-[#003366] border border-white/5 rounded-2xl p-8 space-y-5">
                        ...
                        {['general', 'admissions', 'placements'].map(t => (
                          <button
                            ...
                            className={`py-2 px-3 rounded-xl text-sm capitalize transition-all duration-200 ${
                              form.type === t
                                ? 'bg-[#D32F2F] text-[#003366] font-semibold'
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-[#001A33] border border-white/10 rounded-xl px-4 py-3 text-white text-sm 
                                     focus:outline-none focus:border-[#D32F2F]/50 transition-colors placeholder-gray-600"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full bg-[#001A33] border border-white/10 rounded-xl px-4 py-3 text-white text-sm 
                                   focus:outline-none focus:border-[#D32F2F]/50 transition-colors placeholder-gray-600 resize-none"
                    ...
                    <motion.button
                      type="submit"
                      disabled={loading}
                      className="w-full bg-[#D32F2F] text-[#003366] font-semibold py-4 rounded-xl hover:bg-[#EF5350] 
      ...
      <section className="py-24 bg-[#001122]">
        <div className="max-w-7xl mx-auto px-6">
          <AnimatedSection className="text-center mb-12">
            <span className="text-[#D32F2F] font-mono text-sm tracking-widest uppercase block mb-4">Directory</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {departments_contact.map((d, i) => (
              <AnimatedSection key={d.dept} delay={i * 0.07}>
                <div className="bg-[#003366] border border-white/5 rounded-xl p-5 hover:border-[#D32F2F]/20 transition-all duration-300">
                  <h4 className="text-white font-semibold mb-3">{d.dept}</h4>
                  <div className="space-y-2">
                    <a href={`tel:${d.phone}`} className="flex items-center gap-2 text-gray-400 text-sm hover:text-[#D32F2F] transition-colors">
                      <Phone size={13} className="text-[#D32F2F]" /> {d.phone}
                    </a>
                    <a href={`mailto:${d.email}`} className="flex items-center gap-2 text-gray-400 text-sm hover:text-[#D32F2F] transition-colors">
                      <Mail size={13} className="text-[#D32F2F]" /> {d.email}
                    </a>
                  </div>
                </div>
              </AnimatedSection>
      ...
      {/* Hours */}
      <section className="py-16 bg-[#001A33] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <AnimatedSection className="flex flex-col md:flex-row items-center justify-between gap-6 bg-[#003366] border border-white/5 rounded-2xl px-8 py-6">
            <div className="flex items-center gap-4">
              <Clock size={24} className="text-[#D32F2F]" />
              <div>
            ...
            <div className="text-center">
              <div className="text-[#D32F2F] font-mono text-sm">EMERGENCY CONTACT</div>
              <a href="tel:+918026622130" className="text-white font-semibold text-lg hover:text-[#D32F2F] transition-colors">+91-80-2662 2130</a>
            </div>
